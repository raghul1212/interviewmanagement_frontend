...
export class AdminService {
  ...
  sendScheduledInterviewMail(candidateId:any,empId:any,interview:any):Observable<any>{
    ...
    return this.http.post(`${this.baseUrl1}/scheduled-interview/${candidateId}/${empId}`,interview,{headers,responseType:'text'});
  }

  sendRescheduledInterviewMail(candidateId:any,empId:any,interview:any):Observable<any>{
    const headers=new HttpHeaders().set('content-type','application/json;charset:utf-8');
    return this.http.put(`${this.baseUrl1}/rescheduled-interview/${candidateId}/${empId}`,interview,{headers,responseType:'text'});
  }
  private baseUrl2='http://localhost:8083/result';
  sendResultMail(result:any):Observable<any>{
    const headers=new HttpHeaders().set('content-type','application/json;charset:utf-8');
    return this.http.post(`${this.baseUrl2}/email`,result,{headers,responseType:'text'});
  }
