...
//to update the interview details, we are only updating part of data, so ensure that other data remains same
  updateInterview(interview: Interview) {
    ...
    for (let emp of this.employees) {
      if (emp.id == interview.employee!.id) {//not null assertion is added here 
    ...
    for (let can of this.candidates) {
      if (can.id == interview.candidate!.id) {//not null assertion is added here
  ...
  sendMail(interview: Interview) {
    this.adminService
      .sendRescheduledInterviewMail(interview.candidate!.id, interview.employee!.id, interview)//not null assertion is added here
      .subscribe(
        (data) => {
          this.showSuccess(data.message);
        },
